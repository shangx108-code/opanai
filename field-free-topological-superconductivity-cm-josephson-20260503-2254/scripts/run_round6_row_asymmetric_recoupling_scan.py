...
RESULTS_DIR = PROJECT_ROOT / "results" / "round6-row-asymmetric-recoupling-20260504"
...
class Params:
    t_sc: float = 1.0
    t_junction_x_active: float = 0.43
    t_junction_x_suppressed: float = 0.35
    t_junction_y: float = 0.12
    t_interface_left_active: float = 0.16
    t_interface_right_suppressed: float = 0.16
    alpha_sc: float = 0.42
    alpha_junction_x_active: float = 0.08
    alpha_junction_x_suppressed: float = 0.03
    alpha_junction_y: float = 0.04
    alpha_interface_left_active: float = 0.25
    alpha_interface_right_suppressed: float = 0.06
    delta: float = 0.38
    barrier_active: float = 2.20
    barrier_suppressed: float = 2.50
    orbital_split: float = 0.55
    channel_filter_strength: float = 0.68
    mu_min: float = -2.0
    mu_max: float = 2.0
    mu_count: int = 9
    phi_min: float = 0.0
...
def is_active_row(y: int, params: Params) -> bool:
    return y == params.junction_rows[0]


def channel_weight(y: int, params: Params) -> float:
    if is_active_row(y, params):
        return 1.0
...
def selective_compensated_field(kx: float, y: int, m0: float, params: Params) -> float:
    ...
    sign = 1.0 if is_active_row(y, params) else -1.0
    return sign * weight * m0 * narrow_form


def junction_x_hopping(y: int, params: Params) -> float:
    return params.t_junction_x_active if is_active_row(y, params) else params.t_junction_x_suppressed


def junction_alpha_x(y: int, params: Params) -> float:
    return params.alpha_junction_x_active if is_active_row(y, params) else params.alpha_junction_x_suppressed


def junction_barrier(y: int, params: Params) -> float:
    return params.barrier_active if is_active_row(y, params) else params.barrier_suppressed
...
def build_normal_block(kx: float, mu: float, m0: float, params: Params) -> np.ndarray:
    ...
    for y in range(ny):
        ...
        if stype == "junction":
            t_x = junction_x_hopping(y, params)
            alpha_x = junction_alpha_x(y, params)
            barrier = junction_barrier(y, params)
            extra_shift = orbital_shift(y, params)
            magnetic = selective_compensated_field(kx, y, m0, params) * SZ
        else:
            t_x = params.t_sc
            alpha_x = params.alpha_sc
            barrier = 0.0
            extra_shift = 0.0
    ...
    for y in range(ny - 1):
        ...
        elif right_type == "junction" and is_active_row(y + 1, params):
            t_y = params.t_interface_left_active
            alpha_y = params.alpha_interface_left_active
        elif left_type == "junction" and y == params.junction_rows[1]:
            t_y = params.t_interface_right_suppressed * channel_weight(y, params)
            alpha_y = params.alpha_interface_right_suppressed * channel_weight(y, params)
        else:
            t_y = 0.12
            alpha_y = 0.04
...
def candidate_score(family_traces: dict[tuple[float, float], list[dict[str, float]]]) -> dict[str, float]:
    ...
    pi_wins = 0
    focused_wins = 0
    for trace in family_traces.values():
    ...
    return {
        ...
        "focused_pi_count": float(focused_wins),
    }
...
def main() -> None:
    ...
    candidates = []
    family_rows = []
    for t_interface_left_active, alpha_interface_left_active, alpha_junction_x_active, barrier_active in itertools.product(
        [0.18, 0.20, 0.22],
        [0.26, 0.30, 0.34],
        [0.10, 0.12, 0.14],
        [2.10, 2.20, 2.30],
        ...
        params = replace(
            base,
            t_interface_left_active=t_interface_left_active,
            alpha_interface_left_active=alpha_interface_left_active,
            alpha_junction_x_active=alpha_junction_x_active,
    ...
    best_params = replace(
        base,
        t_interface_left_active=float(best["t_interface_left_active"]),
        alpha_interface_left_active=float(best["alpha_interface_left_active"]),
        alpha_junction_x_active=float(best["alpha_junction_x_active"]),
        barrier_active=float(best["barrier_active"]),
    )
    ...
    evidence_status = "locally promising but still too global"
    if len(near_rows) <= 80 and phi_near_pi_fraction > 0.20:
        evidence_status = "promising asymmetric candidate"

    write_csv(candidates, RESULTS_DIR / "candidate_scores.csv")
    write_csv(family_rows, RESULTS_DIR / "family_gap_traces.csv")
    write_csv(full_rows, RESULTS_DIR / "best_asymmetric_full_scan.csv")
    write_csv(near_rows if near_rows else full_rows[:1], RESULTS_DIR / "best_asymmetric_near_closing.csv")
    ...
    summary = [
        ...
        "## Best candidate",
        "",
        f"- `t_interface_left_active = {best['t_interface_left_active']}`",
        f"- `alpha_interface_left_active = {best['alpha_interface_left_active']}`",
        f"- `alpha_junction_x_active = {best['alpha_junction_x_active']}`",
        f"- `barrier_active = {best['barrier_active']}`",
        f"- family-level `phi~pi` wins: `{int(best['pi_win_count'])}/3`",
        f"- family-level minima at `phi=pi`: `{int(best['focused_pi_count'])}/3`",
