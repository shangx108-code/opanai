...
## 当前唯一主瓶颈
round9 已在当前环境中重新落地并真实运行训练取向比例连续扫参，并把 `x_true` / `x_rev` 限制为 exact ambiguity pair，从而把 solver failure 从 branch selection 中进一步剥离。结果显示：即使 `recovered_measurement_error` 已降到机器精度量级，branch bias 仍会随 training true-orientation ratio 连续跨零翻转。因此，项目当前新的唯一主瓶颈已进一步收缩为：“在去除训练分布取向偏置后，phase branch selection 是否还残留可推广的结构性偏向，还是会收敛到接近零的对称态”。

具体表现：
- round9 共真实完成 `1056` 次 exact-ambiguity branch evaluation，平均 exact ambiguity quantity 为 `1.39e-16`，整体平均 `recovered_measurement_error = 7.11e-17`。
- 在几乎零 solver-error 的条件下，整体平均 `branch_bias = -0.0280`，整体正偏向比例 `0.495`，说明 branch sign 不再能被解释为单侧稳定偏向。
- 当 training true-orientation ratio 从 `0.0` 增加到 `1.0` 时，平均 `branch_bias` 从 `-0.6078` 连续移动到 `0.4125`，正偏向比例从 `0.188` 上升到 `0.729`。
- 在接近对称训练分布的 `ratio = 0.5` 条件下，平均 `branch_bias = -0.0894`，正偏向比例 `0.479`；在 `ratio = 0.6` 时已转为平均 `branch_bias = 0.1951`，正偏向比例 `0.583`。
- 这说明 training orientation bias 本身已足以在 exact ambiguity benchmark 上连续驱动 branch sign 变化，因此“branch sign 稳定且可推广”目前没有证据支撑。
- DSI / PDR / HCI 仍未完成完整推导、完整适用边界说明和可计算实现。
- 正文与补充材料仍无投稿级图表体系。

## 本轮唯一最高优先级
在 round9 已确认 orientation ratio 本身即可连续驱动 branch sign 跨零翻转后，本轮新的唯一最高优先级是：补一个去偏置或后验平均型 phase baseline，真实检验在对称先验下 residual branch bias 是否收敛到接近零。

## 本轮交付物
1. 当前工作区可现场复核的 `round9_phase_orientation_ratio_scan.py`
2. `round9_phase_orientation_ratio_outputs/` 真实输出目录
3. `round9_phase_case_metrics.csv`、`round9_phase_summary.json` 与 `round9_phase_panel.png`
4. “orientation ratio 可在 exact ambiguity benchmark 上连续驱动 branch bias 跨零翻转”的优先级判定
5. 更新后的项目状态、监督结论与预审判断

## 完成标准
- 当前环境已真实跑出 orientation ratio 连续扫参
- 扫参结果字段仍与 round6 的 exact / empirical 三层量兼容
- 已在接近零 solver-error 条件下判断 branch bias 是否随 orientation bias 连续响应
- 已据此把下一轮唯一动作收缩为“去偏置基线是否使 residual bias 归零”

## 下一轮立即动作
1. 在当前 exact-ambiguity benchmark 上补一个去偏置或后验平均型 phase baseline，直接测试 symmetric prior 下的 residual branch bias。
2. 若 residual bias 接近零，再把当前 phase 写法收紧为“orientation-biased prior induces controllable symmetry breaking”。
3. 若 residual bias 仍显著偏离零，再继续排查 architecture bias、score parameterization 或优化器隐式偏置。
...
当前最拖累接收概率的短板：
1. 当前虽已证明 orientation bias 足以连续驱动 branch sign，但尚未证明在对称先验下 residual branch bias 会消失，因此机制归因仍未闭合
