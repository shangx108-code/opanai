...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Scan non-A-B mixed-star phase conventions on top of the best exact-Gamma k-path baseline."
    )
    parser.add_argument("--output-dir", type=Path, default=Path("/workspace/output/twse2_mixed_star_scan"))
...
def main() -> None:
    ...
    for c1_pattern, c2_pattern, c7_first_pattern, c7_second_pattern, c7_second_conjugated in product(
        PHASE_PATTERNS, PHASE_PATTERNS, PHASE_PATTERNS, PHASE_PATTERNS, [False, True]
    ):
        mixed_star_config = {
            "ac1_pattern": c1_pattern,
            "bc1_pattern": c1_pattern,
            "ac2_pattern": c2_pattern,
            "bc2_pattern": c2_pattern,
            "ac7_first_pattern": c7_first_pattern,
            "bc7_first_pattern": c7_first_pattern,
            "ac7_second_pattern": c7_second_pattern,
            "bc7_second_pattern": c7_second_pattern,
            "ac7_second_conjugated": c7_second_conjugated,
            "bc7_second_conjugated": c7_second_conjugated,
        }
        ...
        row = {
            ...
            "c7_first_pattern": c7_first_pattern,
            "c7_second_pattern": c7_second_pattern,
            "c7_second_conjugated": c7_second_conjugated,
    ...
    ledger.to_csv(out_dir / "mixed_star_scan.csv", index=False)
    ledger.head(20).to_csv(out_dir / "mixed_star_scan_top20.csv", index=False)
    ...
    summary_lines = [
        ...
        f"- c1 pattern: {best_overall_result['mixed_star_config']['ac1_pattern']}",
        f"- c2 pattern: {best_overall_result['mixed_star_config']['ac2_pattern']}",
        f"- c7 first-orbit pattern: {best_overall_result['mixed_star_config']['ac7_first_pattern']}",
        f"- c7 second-orbit pattern: {best_overall_result['mixed_star_config']['ac7_second_pattern']}",
        f"- c7 second-orbit conjugated: {best_overall_result['mixed_star_config']['ac7_second_conjugated']}",
    ...
    if best_exact_gamma_result is None:
        summary_lines.extend(
            [
                ...
                "- none within the current acceptance threshold",
                "",
            ]
        )
    else:
        summary_lines.extend(
            [
                "## Best exact-Gamma candidate",
                f"- c1 pattern: {best_exact_gamma_result['mixed_star_config']['ac1_pattern']}",
                f"- c2 pattern: {best_exact_gamma_result['mixed_star_config']['ac2_pattern']}",
                f"- c7 first-orbit pattern: {best_exact_gamma_result['mixed_star_config']['ac7_first_pattern']}",
                f"- c7 second-orbit pattern: {best_exact_gamma_result['mixed_star_config']['ac7_second_pattern']}",
                f"- c7 second-orbit conjugated: {best_exact_gamma_result['mixed_star_config']['ac7_second_conjugated']}",
