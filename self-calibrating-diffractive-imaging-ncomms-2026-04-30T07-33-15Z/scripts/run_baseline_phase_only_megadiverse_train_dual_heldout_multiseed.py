#!/usr/bin/env python3
"""Run fixed dual-heldout ledgers after one more large training-family expansion."""
...
def make_megadiverse_training_objects(size: int) -> list[np.ndarray]:
    ...
    for name in names:
        ...
        if name == "diamond":
            ...
            for step, (x0, y0, x1, y1) in enumerate([(18,90,110,108),(28,72,100,90),(38,54,90,72),(48,36,80,54)]): draw.rectangle((x0,y0,x1,y1), fill=255)
...
def main() -> int:
    ...
    out_dir = project_root / "results" / "baselines" / "baseline-008-phaseonly-megadiverse-train-dual-heldout-multiseed"
