#!/usr/bin/env python3
"""Run higher-capacity phase-only dual-heldout evaluation with expanded training families."""
...
def make_highcap_training_objects(size: int) -> list[np.ndarray]:
    ...
    for name in names:
        ...
        if name == "diamond":
            ...
            draw.rectangle((24, 22, 104, 42), fill=255)
            draw.rectangle((54, 22, 74, 106), fill=255)
        elif name == "frame":
            draw.rectangle((22, 22, 106, 106), outline=255, width=10)
        elif name == "chevron":
            draw.line((26, 40, 64, 84), fill=255, width=12)
            draw.line((64, 84, 102, 40), fill=255, width=12)
            draw.line((26, 76, 64, 112), fill=255, width=12)
            draw.line((64, 112, 102, 76), fill=255, width=12)
        elif name == "triple_dots":
            draw.ellipse((18, 46, 42, 70), fill=255)
            draw.ellipse((52, 46, 76, 70), fill=255)
            draw.ellipse((86, 46, 110, 70), fill=255)
        elif name == "ladder":
            draw.rectangle((28, 20, 38, 108), fill=255)
            draw.rectangle((90, 20, 100, 108), fill=255)
            for y in [30, 48, 66, 84]:
                draw.rectangle((38, y, 90, y + 8), fill=255)
        elif name == "offcenter_ring":
            draw.ellipse((18, 28, 94, 104), outline=255, width=10)
            draw.ellipse((44, 44, 68, 68), fill=255)
        elif name == "fork":
            draw.rectangle((56, 24, 72, 108), fill=255)
            draw.rectangle((36, 20, 50, 54), fill=255)
            draw.rectangle((78, 20, 92, 54), fill=255)
        elif name == "plus_disk":
            draw.ellipse((34, 34, 94, 94), fill=255)
            draw.rectangle((56, 18, 72, 110), fill=255)
            draw.rectangle((18, 56, 110, 72), fill=255)
        elif name == "zigzag":
            points = [(20, 34), (48, 62), (34, 76), (62, 104), (90, 76), (76, 62), (104, 34)]
            draw.line(points, fill=255, width=12)
        elif name == "two_boxes":
            draw.rectangle((22, 32, 54, 96), outline=255, width=8)
            draw.rectangle((74, 32, 106, 96), outline=255, width=8)
        elif name == "hourglass":
            draw.polygon([(28, 26), (54, 26), (76, 60), (98, 26), (100, 28), (76, 64), (100, 100), (98, 102), (76, 68), (54, 102), (28, 102), (52, 64)], fill=255)
        elif name == "arch":
            draw.arc((18, 20, 110, 108), start=180, end=360, fill=255, width=12)
            draw.rectangle((24, 62, 38, 108), fill=255)
            draw.rectangle((90, 62, 104, 108), fill=255)
        elif name == "bracket_pair":
            draw.rectangle((22, 24, 34, 106), fill=255)
            draw.rectangle((22, 24, 60, 36), fill=255)
            draw.rectangle((22, 94, 60, 106), fill=255)
            draw.rectangle((94, 24, 106, 106), fill=255)
            draw.rectangle((68, 24, 106, 36), fill=255)
            draw.rectangle((68, 94, 106, 106), fill=255)
        elif name == "shifted_cross":
            draw.rectangle((48, 14, 62, 104), fill=255)
            draw.rectangle((18, 50, 98, 64), fill=255)
            draw.rectangle((74, 34, 90, 116), fill=255)
        elif name == "quad_disks":
            for x0, y0 in [(20, 20), (72, 20), (20, 72), (72, 72)]:
                draw.ellipse((x0, y0, x0 + 28, y0 + 28), fill=255)
        objects.append(np.asarray(canvas, dtype=np.float64) / 255.0)
    return objects
...
def optimize_phase_coeffs(
    ...
    for _ in range(config.search_steps):
        ...
        loss_plus = evaluate_training_objective(
            candidate_plus, basis, train_samples, propagation, config.ridge_lambda
        )
        loss_minus = evaluate_training_objective(
...
def run_seed(config: PhaseOnlyConfig) -> tuple[dict[str, tuple[pd.DataFrame, dict[str, float]]], dict[str, float]]:
    train_objects = make_highcap_training_objects(config.image_size)
    ...
    x_same = build_features(same_family_samples, masks, propagation)
    x_new = build_features(new_family_samples, masks, propagation)
    same_prediction = predict_ridge(x_same, weights)
    new_prediction = predict_ridge(x_new, weights)

    protocol_outputs = {
        "same_family_heldout_aberration": evaluate_protocol(
            same_family_samples, same_prediction, config, config.seed, "same_family_heldout_aberration"
        ),
        "new_family_heldout_object_family": evaluate_protocol(
            new_family_samples, new_prediction, config, config.seed, "new_family_heldout_object_family"
        ),
    }
    ...
    shared_metadata = {
        ...
        "lowres_size": config.lowres_size,
        "training_object_family_count": len(train_objects),
        "same_family_eval_count": len(same_family_eval_objects),
        "new_family_eval_count": len(object_family_heldout),
        "wavelength_m": config.wavelength,
        "sample_spacing_m_native": config.sample_spacing,
        "sample_spacing_m_frontend": frontend_spacing,
...
def write_protocol_log(log_path: Path, shared: dict[str, float], protocol_summary: dict[str, float]) -> None:
    log_path.write_text(
        "\n".join(
            [
                f"seed={shared['seed']}",
                f"protocol={protocol_summary['protocol']}",
                "propagation_model=fresnel",
                "fresnel_transfer_function=exp(-i*pi*lambda*z*(fx^2+fy^2))",
                f"layer_count={shared['layer_count']}",
                f"phase_basis_count={shared['phase_basis_count']}",
                f"lowres_size={shared['lowres_size']}",
                f"training_object_family_count={shared['training_object_family_count']}",
                f"same_family_eval_count={shared['same_family_eval_count']}",
                f"new_family_eval_count={shared['new_family_eval_count']}",
                f"wavelength_m={shared['wavelength_m']}",
                f"sample_spacing_m_native={shared['sample_spacing_m_native']}",
                f"sample_spacing_m_frontend={shared['sample_spacing_m_frontend']}",
                f"propagation_distance_m={shared['propagation_distance_m']}",
                f"fx_min_1_per_m={shared['fx_min_1_per_m']}",
                f"fx_max_1_per_m={shared['fx_max_1_per_m']}",
                f"fy_min_1_per_m={shared['fy_min_1_per_m']}",
                f"fy_max_1_per_m={shared['fy_max_1_per_m']}",
                f"mean_phaseonly_psnr_lowres={protocol_summary['mean_phaseonly_psnr_lowres']}",
                f"mean_phaseonly_ssim_lowres={protocol_summary['mean_phaseonly_ssim_lowres']}",
                f"phaseonly_psnr_gain_over_fixed_lowres={protocol_summary['mean_phaseonly_psnr_gain_over_fixed_lowres']}",
                f"phaseonly_better_than_fixed_fraction={protocol_summary['phaseonly_better_than_fixed_fraction']}",
            ]
        )
        + "\n",
        encoding="utf-8",
...
def main() -> int:
    ...
    out_dir = project_root / "results" / "baselines" / "baseline-006-phaseonly-highcap-expanded-train-dual-heldout-multiseed"
    ...
    generated_by_protocol: dict[str, list[Path]] = {name: [] for name in protocol_names}
    per_seed_summaries: dict[str, list[dict[str, float]]] = {name: [] for name in protocol_names}
    ...
    for protocol_name in protocol_names:
        ...
        manifest_path = protocol_dir / "sha256_manifest.txt"
        write_sha256_manifest(generated_by_protocol[protocol_name], manifest_path)
        ...
        json.dumps(
            {
                ...
                "training_object_family_count": 22,
                "layer_count": 5,
