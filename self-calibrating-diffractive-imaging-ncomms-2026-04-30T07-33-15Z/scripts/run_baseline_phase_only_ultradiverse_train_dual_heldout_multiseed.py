#!/usr/bin/env python3
"""Run fixed dual-heldout ledgers after further expanding training-family diversity."""
...
def make_ultradiverse_training_objects(size: int) -> list[np.ndarray]:
    objects = list(make_objects(size))
    names = [
        "diamond", "t_shape", "frame", "chevron", "triple_dots", "ladder", "offcenter_ring", "fork",
        "plus_disk", "zigzag", "two_boxes", "hourglass", "arch", "bracket_pair", "shifted_cross",
        "quad_disks", "tilted_bar_pair", "triangle_ring", "donut_bar", "staggered_blocks", "y_shape",
        "split_circle", "notched_frame", "spiral_stub", "offset_triplet", "l_shape", "u_shape",
        "kite", "double_chevron", "windowpane"
    ]
    for name in names:
        canvas = Image.new("L", (size, size), 0)
        draw = ImageDraw.Draw(canvas)
        if name == "diamond":
            ...
            for i, (x, y) in enumerate([(18, 28), (42, 52), (66, 28), (90, 52)]): draw.rectangle((x, y, x + 18, y + 34), fill=255)
        elif name == "y_shape":
...
def run_seed(config: PhaseOnlyConfig) -> tuple[dict[str, tuple[pd.DataFrame, dict[str, float]]], dict[str, float]]:
    train_objects = make_ultradiverse_training_objects(config.image_size)
...
def main() -> int:
    ...
    out_dir = project_root / "results" / "baselines" / "baseline-007-phaseonly-ultradiverse-train-dual-heldout-multiseed"
    ...
    print(json.dumps({
        ...
        "training_object_family_count": 36,
        "layer_count": 5,
