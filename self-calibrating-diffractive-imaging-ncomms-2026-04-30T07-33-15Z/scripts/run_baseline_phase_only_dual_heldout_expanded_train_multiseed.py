#!/usr/bin/env python3
"""Run dual-ledger held-out evaluation after expanding the training object family."""
...
def make_expanded_training_objects(size: int) -> list[np.ndarray]:
...
def run_seed(
    config: PhaseOnlyConfig,
) -> tuple[dict[str, tuple[pd.DataFrame, dict[str, float]]], dict[str, float]]:
    train_objects = make_expanded_training_objects(config.image_size)
    ...
    same_family_samples = build_split_samples(
        same_family_eval_objects, "heldout_aberration", heldout_cases, config
    )
    new_family_samples = build_split_samples(
        object_family_heldout, "heldout_object_family", heldout_cases, config
    )
...
def main() -> int:
    ...
    out_dir = project_root / "results" / "baselines" / "baseline-005-phaseonly-expanded-train-dual-heldout-multiseed"
        ...
        json.dumps(
            {
                ...
                "training_object_family_count": 14,
                "layer_count": 3,
            },
            indent=2,
        )
