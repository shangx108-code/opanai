...
def build_hops() -> dict[tuple[str, str, tuple[float, float]], complex]:
    ...
    def add_mixed_star(key: tuple[str, str, float], amplitude: complex, second_orbit_conjugated: bool = False) -> None:
        ...
        for vec in first_orbit:
            ...
            current *= OMEGA
        if len(vecs) == 6:
            current = np.conj(amplitude) if second_orbit_conjugated else amplitude
            for vec in vecs[3:]:
                hops[(alpha, beta, tuple(np.round(vec, 6)))] = current
                hops[(beta, alpha, tuple(np.round(-vec, 6)))] = np.conj(current)
                current *= OMEGA
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--k-b-label", default="K_2b1+b2")
    parser.add_argument("--m-label", default="M_b1+b2")
    parser.add_argument("--k-t-label", default="K_b1+2b2")
    parser.add_argument("--source-file", type=Path, default=DEFAULT_SOURCE_FILE)
    parser.add_argument("--fallback-file", type=Path, default=DEFAULT_FALLBACK_FILE)
    return parser.parse_args()


def main() -> None:
    args = parse_args()
    source, _ = load_source(args.source_file, args.fallback_file)
    result = evaluate_candidate(args.path_mode, args.k_b_label, args.m_label, args.k_t_label, source=source)
    result["source_origin"] = load_source(args.source_file, args.fallback_file)[1]
