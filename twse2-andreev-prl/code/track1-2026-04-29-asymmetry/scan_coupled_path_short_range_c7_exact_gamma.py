...
DEFAULT_SHORT_RANGE_LEDGER = Path(
    "/workspace/memory/twse2-andreev-prl/data/track1-2026-04-29-asymmetric-short-range/asymmetric_short_range_scan.csv"
)
DEFAULT_C7_LEDGER = Path(
    "/workspace/memory/twse2-andreev-prl/data/track1-2026-04-29-asymmetric-c7/asymmetric_c7_scan.csv"
)
DEFAULT_OUTPUT_DIR = Path("/workspace/output/twse2_coupled_path_short_range_c7_scan")


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Jointly scan top k-path mappings, exact-Gamma short-range asymmetry branches, "
            "and exact-Gamma sqrt(7) asymmetry branches for the next coupled Track-1 layer."
        )
    )
    parser.add_argument("--kpath-ledger", type=Path, default=DEFAULT_KPATH_LEDGER)
    parser.add_argument("--short-range-ledger", type=Path, default=DEFAULT_SHORT_RANGE_LEDGER)
    ...
    parser.add_argument("--top-short-range", type=int, default=6)
    parser.add_argument("--top-c7", type=int, default=20)
...
def load_exact_gamma_short_range(path: Path, threshold: float, top_n: int) -> pd.DataFrame:
    ledger = pd.read_csv(path)
...
def load_exact_gamma_c7(path: Path, threshold: float, top_n: int) -> pd.DataFrame:
    ledger = pd.read_csv(path)
    filtered = ledger[ledger["gamma_end_max_abs_delta_meV"] <= threshold].copy()
    ranked = filtered.sort_values(
        ["overall_rmse_meV", "gamma_end_max_abs_delta_meV", "k_b_window_rmse_meV", "k_t_window_rmse_meV"]
...
def build_mixed_star_config(short_range_row: pd.Series, c7_row: pd.Series) -> dict[str, object]:
    return {
        "ac1_pattern": str(short_range_row["ac1_pattern"]),
        "bc1_pattern": str(short_range_row["bc1_pattern"]),
        "ac2_pattern": str(short_range_row["ac2_pattern"]),
        "bc2_pattern": str(short_range_row["bc2_pattern"]),
...
def main() -> None:
    ...
    exact_gamma_short_range = load_exact_gamma_short_range(
        args.short_range_ledger, args.gamma_threshold_meV, args.top_short_range
    )
    exact_gamma_c7 = load_exact_gamma_c7(args.c7_ledger, args.gamma_threshold_meV, args.top_c7)
    if top_kpaths.empty:
        raise RuntimeError("No k-path candidates were loaded.")
    if exact_gamma_short_range.empty:
        raise RuntimeError("No exact-Gamma short-range candidates are available for the coupled scan.")
    ...
    for path_rank, kpath_row in top_kpaths.iterrows():
        for short_rank, short_row in exact_gamma_short_range.iterrows():
            for c7_rank, c7_row in exact_gamma_c7.iterrows():
                mixed_star_config = build_mixed_star_config(short_row, c7_row)
                ...
                row = {
                    ...
                    "short_range_rank": int(short_rank + 1),
                    ...
                    "ac1_pattern": str(short_row["ac1_pattern"]),
                    "bc1_pattern": str(short_row["bc1_pattern"]),
                    "ac2_pattern": str(short_row["ac2_pattern"]),
                    "bc2_pattern": str(short_row["bc2_pattern"]),
                    "ac7_first_pattern": str(c7_row["ac7_first_pattern"]),
                    "bc7_first_pattern": str(c7_row["bc7_first_pattern"]),
                    "ac7_second_pattern": str(c7_row["ac7_second_pattern"]),
                    "bc7_second_pattern": str(c7_row["bc7_second_pattern"]),
    ...
    ledger.to_csv(out_dir / "coupled_path_short_range_c7_scan.csv", index=False)
    ledger.head(20).to_csv(out_dir / "coupled_path_short_range_c7_scan_top20.csv", index=False)
    ...
    summary_lines = [
        "# Coupled Path + Short-Range + Asymmetric sqrt(7) Exact-Gamma Scan Summary",
        "",
        f"- Source origin: {source_origin}",
        f"- K-path ledger: {args.kpath_ledger}",
        f"- Short-range ledger: {args.short_range_ledger}",
        f"- Asymmetric sqrt(7) ledger: {args.c7_ledger}",
        f"- Top k-path candidates included: {len(top_kpaths)}",
        f"- Exact-Gamma short-range candidates included: {len(exact_gamma_short_range)}",
    ...
    summary_lines.extend(
        [
            "Generated files:",
            "- coupled_path_short_range_c7_scan.csv",
            "- coupled_path_short_range_c7_scan_top20.csv",
        ]
    )
    if best_exact_gamma_result is not None:
        summary_lines.extend(
